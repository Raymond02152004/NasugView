...
<div class="container-fluid px-3">
  <div class="card card-custom">
    ...
    <form action="{{ route('negosyo.events.update', $form->form_id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="card-section">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" name="description" rows="3" placeholder="Describe your event">{{ $form->description }}</textarea>
            </div>
        </div>
          ...
          @foreach ($form->questions as $index => $question)
            <div class="card-section">
              <div class="mb-2">
                ...
                <input type="text" class="form-control" name="questions[{{ $index + 1 }}][text]" value="{{ $question->question_text }}" required>
              </div>
              <div class="mb-2">
                <label class="form-label fw-semibold">Question Type</label>
                <select class="form-select" name="questions[{{ $index + 1 }}][type]" onchange="toggleChoices(this, {{ $index + 1 }})" required>
                  <option value="text" {{ $question->question_type == 'text' ? 'selected' : '' }}>Text Answer</option>
                  <option value="multiple_choice" {{ $question->question_type == 'multiple_choice' ? 'selected' : '' }}>Multiple Choice</option>
                </select>
              </div>
              <div class="choices-container {{ $question->question_type == 'multiple_choice' ? '' : 'd-none' }}" id="choices-{{ $index + 1 }}">
                <label class="form-label fw-semibold">Choices</label>
                <div class="choices-list">
                  @foreach ($question->choices as $choiceIndex => $choice)
                    <div class="input-group mb-2">
                      <span class="input-group-text">Choice #{{ $choiceIndex + 1 }}</span>
                      <input type="text" class="form-control" name="questions[{{ $index + 1 }}][choices][]" value="{{ $choice->choice_text }}" required>
                    </div>
                  @endforeach
                </div>
                <button type="button" class="btn btn-sm btn-secondary mt-2" onclick="addChoice({{ $index + 1 }})">Add Choice</button>
              </div>
            </div>
          @endforeach
        </div>
        ...
        <div class="d-flex justify-content-end mt-4">
            ...
            <a href="{{ route('negosyo.events.index') }}" class="btn btn-secondary ms-2">Cancel</a>
        </div>
    </form>
...
<script>
    let questionCount = {{ count($form->questions) }};
